document.addEventListener('DOMContentLoaded', () => {
    ...
    /* === Validações extras para erros ===

    === Erro acima do campo dinamicamente com JS:

    fullNameInput.parentNode.insertAdjacentHTML(
        "afterbegin",
        `<small class="error-message">Digite seu nome</small>`
    );
    ...
    */  
    ...
    function validateForm() {
        ...
        === Validar apenas o primeiro nome sem regex: 

        if (fullNameInput.value.trim().split(" ").length > 1) {
            isValid = false;
            showError(fullNameInput, "Digite apenas o primeiro nome.");
        }
        ...
        if (parts.length < 2) {
            isValid = false;
            showError(fullNameInput, "Digite nome e sobrenome.");
        }
    ...
    === Obrigar a escrever com “@”:

    if (!githubInput.value.startsWith("@")) {
        isValid = false;
        showError(githubInput, "O usuário do GitHub deve começar com '@'.");
    }

    === Não permitir @, é adicionar automaticamente: 

    let username = githubInput.value.replace("@", "");
    githubInput.value = "@" + username;

    === Independente de como o usuário escreveu, não duplicar o “@”: 
    ...
    */
    ...
    === Validação máxima e mínima de um nome:
    
    if (fullNameInput.value.length < 3) {
        showError(fullNameInput, "O nome deve ter pelo menos 3 caracteres.");
    }

    === Validação de domínio de e-mail, vai aceitar só gmail @:

    if (!emailInput.value.endsWith("@gmail.com")) {
        showError(emailInput, "O email deve ser do Gmail.");
    }
